...
databricks catalogs list --profile rota-perfume | grep rotaperfume || echo 'não existe'
...
```
Leia CLAUDE.md (raiz do repositório) antes de começar.

Crie o projeto da noite 2 em rotaperfume/, na raiz do repositório,
...
- profile: rota-perfume   (sempre passe --profile, nunca deixe implícito)
- host: https://dbc-49ad8178-eeec.cloud.databricks.com
- SQL Warehouse: 869f5987fba5e50f (Serverless Starter Warehouse)
- tudo serverless, nunca configure cluster

1. databricks.yml
   - bundle name: rotaperfume
   - variables: catalog (default lakehouse_rotaperfume) e warehouse_id
     (default 869f5987fba5e50f)
...
2. scripts/criar-catalogo.sh
   ...
   POR QUE NÃO ESTÁ NO BUNDLE: nesta conta o Default Storage está ligado, e
   nessa configuração a API do Unity Catalog RECUSA criar catálogo pelo bundle
   — ela exige uma MANAGED LOCATION explícita:
...
```bash
databricks fs ls dbfs:/Volumes/lakehouse_rotaperfume/bronze/raw/erp --profile rota-perfume
```
...
databricks fs rm dbfs:/Volumes/lakehouse_rotaperfume/bronze/raw/erp/pagamentos.csv \
  --profile rota-perfume
databricks bundle run rotaperfume_pipeline --target dev --profile rota-perfume
# a tarefa raw_conferencia FALHA e o job para: falta pagamentos.csv
bash scripts/subir-raw.sh rota-perfume     # devolve o arquivo e rode de novo
```
